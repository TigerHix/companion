...
const inputGroupAddonVariants = cva(
  "text-muted-foreground h-auto gap-2 py-1.5 text-sm font-medium group-data-[disabled=true]/input-group:opacity-50 [&>svg:not([class*='size-'])]:size-4 flex cursor-text items-center justify-center select-none",
  {
    variants: {
      align: {
        "inline-start":
          "pl-2 has-[>button]:ml-[-0.25rem] order-first",
        "inline-end":
          "pr-2 has-[>button]:mr-[-0.25rem] order-last",
        "block-start":
...
function InputGroupInput({
  ...
  return (
    <Input
      ...
      className={cn(
        "!bg-transparent !border-0 !shadow-none !rounded-none flex-1",
        className
      )}
      {...props}
    />
...
function InputGroupTextarea({
  ...
  return (
    <Textarea
      data-slot="input-group-control"
      className={cn(
        "!bg-transparent !border-0 !shadow-none !rounded-none py-2 flex-1 resize-none",
        className
      )}
      {...props}
    />

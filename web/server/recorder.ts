 ...
 *
 * Always enabled by default. Disable explicitly with MOKU_RECORD=0.
 *
 * Automatic rotation: when total lines across all recording files exceed
 * maxLines (default 1 000 000, override with MOKU_RECORDINGS_MAX_LINES),
...
export class RecorderManager {
  ...
  constructor(options?: {
    ...
    this.recordingsDir =
      ...
      process.env.MOKU_RECORDINGS_DIR ??
      join(homedir(), ".moku", "recordings");
    this.maxLines =
      options?.maxLines ??
      (Number(process.env.MOKU_RECORDINGS_MAX_LINES) || DEFAULT_MAX_LINES);
  ...
  /**
   * Always on unless explicitly disabled with MOKU_RECORD=0|false.
   */
  private static resolveEnabled(): boolean {
    const env = process.env.MOKU_RECORD;

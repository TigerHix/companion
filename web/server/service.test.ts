...
function plistPath(): string {
  return join(tempDir, "Library", "LaunchAgents", "app.moku.dev.plist");
}

function oldPlistPath(): string {
  return join(tempDir, "Library", "LaunchAgents", "app.moku.dev.legacy.plist");
}

function unitPath(): string {
  return join(tempDir, ".config", "systemd", "user", "moku.service");
}

function logDir(): string {
  return join(tempDir, ".moku", "logs");
}
...
describe("generatePlist", () => {
  it("generates valid XML with the correct label", () => {
    const plist = service.generatePlist({ binPath: "/usr/local/bin/moku" });
    expect(plist).toContain('<?xml version="1.0"');
    expect(plist).toContain("<string>app.moku.dev</string>");
  });
  ...
  it("uses the provided binary path in ProgramArguments", () => {
    const plist = service.generatePlist({ binPath: "/opt/homebrew/bin/moku" });
    expect(plist).toContain("<string>/opt/homebrew/bin/moku</string>");
  ...
  it("uses a custom port when specified", () => {
    const plist = service.generatePlist({ binPath: "/usr/local/bin/moku", port: 8080 });
  ...
  it("uses custom path option when provided", () => {
    ...
    const plist = service.generatePlist({ binPath: "/usr/local/bin/moku", path: customPath });
...
describe("generateSystemdUnit", () => {
  it("generates a valid systemd unit with correct sections", () => {
    const unit = service.generateSystemdUnit({ binPath: "/usr/local/bin/moku" });
  ...
  it("includes the description", () => {
    const unit = service.generateSystemdUnit({ binPath: "/usr/local/bin/moku" });
    expect(unit).toContain("Description=Moku");
  });

  it("uses the provided binary path in ExecStart", () => {
    const unit = service.generateSystemdUnit({ binPath: "/home/user/.bun/bin/moku" });
    expect(unit).toContain("ExecStart=/home/user/.bun/bin/moku start --foreground");
  });
  ...
  it("uses a custom port when specified", () => {
    const unit = service.generateSystemdUnit({ binPath: "/usr/local/bin/moku", port: 8080 });
  ...
  it("uses custom path option when provided", () => {
    const customPath = "/custom/bin:/other/bin";
    const unit = service.generateSystemdUnit({ binPath: "/usr/local/bin/moku", path: customPath });
...
describe("install", () => {
  it("creates log directory and writes plist file", async () => {
    mockExecSync.mockImplementation((cmd: string) => {
      if (cmd.startsWith("which")) return "/usr/local/bin/moku\n";
    ...
    expect(content).toContain("app.moku.dev");
  });
  ...
  it("migrates old launchd label before installing", async () => {
    ...
    const plist = service.generatePlist({ binPath: "/usr/local/bin/moku" })
      .replaceAll("app.moku.dev", "app.moku.dev.legacy");
...
describe("install (linux)", () => {
  ...
  it("creates log directory and writes systemd unit file", async () => {
    ...
    expect(content).toContain("ExecStart=/usr/local/bin/moku start --foreground");
  });
...
describe("start (linux)", () => {
  ...
  it("calls systemctl start when installed", async () => {
    ...
    const startCall = mockExecSync.mock.calls.find(
      ([cmd]) => typeof cmd === "string" && cmd.includes("start moku.service"),
    );
    expect(startCall).toBeDefined();
  });
...
describe("stop (linux)", () => {
  ...
  it("calls systemctl stop when installed", async () => {
    ...
    const stopCall = mockExecSync.mock.calls.find(
      ([cmd]) => typeof cmd === "string" && cmd.includes("stop moku.service"),
    );
    expect(stopCall).toBeDefined();
  });
...
describe("restart (linux)", () => {
  ...
  it("calls systemctl restart when installed", async () => {
    ...
    const restartCall = mockExecSync.mock.calls.find(
      ([cmd]) => typeof cmd === "string" && cmd.includes("restart moku.service"),
    );
    expect(restartCall).toBeDefined();
  });
...
describe("status (linux)", () => {
  ...
  it("returns installed: true, running: true with PID", async () => {
    ...
    mockExecSync.mockReset();
    mockExecSync.mockImplementation((cmd: string) => {
      if (typeof cmd === "string" && cmd.includes("show moku.service")) {
...
describe("refreshServiceDefinition (macOS)", () => {
  it("rewrites plist with current binary path", async () => {
    ...
    expect(originalContent).toContain("/usr/local/bin/moku");
    ...
    mockExecSync.mockReset();
    mockExecSync.mockImplementation((cmd: string) => {
      if (cmd.startsWith("which")) return "/new/path/moku\n";
      return "";
    });
    ...
    expect(updatedContent).toContain("/new/path/moku");
  });

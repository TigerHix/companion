...
// Use a temp directory so tests don't touch the real ~/.moku/auth.json
const TEST_DIR = join(tmpdir(), `moku-auth-test-${Date.now()}`);
...
describe("auth-manager", () => {
  ...
  beforeEach(async () => {
    ...
    // Clear env var
    delete process.env.MOKU_AUTH_TOKEN;
  ...
  it("uses MOKU_AUTH_TOKEN env var when set", () => {
    // Env var should override any persisted or generated token
    process.env.MOKU_AUTH_TOKEN = "my-custom-token-123";
    authManager._resetForTest();
  ...
  it("verifyToken works with env var token", () => {
    process.env.MOKU_AUTH_TOKEN = "env-token-abc";
    authManager._resetForTest();

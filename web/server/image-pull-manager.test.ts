...
const mockGetRegistryImage = vi.hoisted(() => vi.fn((tag: string) => {
  if (tag === "moku:latest") return "docker.io/moku/moku:latest";
...
describe("ImagePullManager", () => {
  ...
  describe("getState", () => {
    it("returns 'ready' when image exists locally", async () => {
      ...
      const state = manager.getState("moku:latest");
      expect(state.status).toBe("ready");
      expect(state.image).toBe("moku:latest");
    });
  ...
  describe("isReady", () => {
    it("returns true when image exists locally", async () => {
      mockImageExists.mockReturnValue(true);
      const manager = await createManager();
      expect(manager.isReady("moku:latest")).toBe(true);
    });

    it("returns false when image does not exist locally", async () => {
      mockImageExists.mockReturnValue(false);
      const manager = await createManager();
      expect(manager.isReady("moku:latest")).toBe(false);
    });
  });
  ...
  describe("ensureImage", () => {
    it("starts a background pull when image is missing", async () => {
      ...
      manager.ensureImage("moku:latest");
      ...
      await vi.waitFor(() => {
        expect(manager.getState("moku:latest").status).toBe("ready");
      });
    ...
    it("is a no-op when image is already being pulled", async () => {
      ...
      manager.ensureImage("moku:latest");
      manager.ensureImage("moku:latest"); // second call
  ...
  describe("waitForReady", () => {
    it("resolves immediately when image already exists", async () => {
      ...
      const result = await manager.waitForReady("moku:latest");
    ...
    it("waits for an in-progress pull to complete", async () => {
      ...
      manager.ensureImage("moku:latest");
      const result = await manager.waitForReady("moku:latest", 5000);
      expect(result).toBe(true);
    });
    ...
    it("times out when pull takes too long", async () => {
      ...
      const result = await manager.waitForReady("moku:latest", 50);
      expect(result).toBe(false);
    });
  });
  ...
  describe("onProgress", () => {
    it("fires callback for each progress line during pull", async () => {
      ...
      const manager = await createManager();
      const unsub = manager.onProgress("moku:latest", (line) => lines.push(line));
  ...
  describe("fallback to local build", () => {
    it("falls back to local build when pull fails for moku:latest", async () => {
      mockImageExists.mockReturnValue(false);
      mockPullImage.mockResolvedValue(false);
      mockExistsSync.mockReturnValue(true);
  ...
  describe("initFromEnvironments", () => {
    it("pre-pulls missing images for all environments", async () => {
      ...
      mockListEnvs.mockReturnValue([
        { name: "env1", slug: "env1", baseImage: "moku:latest", variables: {}, createdAt: 0, updatedAt: 0 },
      ...
      // moku:latest has a registry mapping, so it's pulling
      const state1 = manager.getState("moku:latest");
